...
func (b *Bindings) GenerateGoBindings(baseDir string) error {
	...
	for packageName, structs := range store {
		...
		for structName, methods := range structs {
			...
			gooutput.WriteString(`// Cynhyrchwyd y ffeil hon yn awtomatig. PEIDIWCH Â MODIWL
			// This file is automatically generated. DO NOT EDIT`)
			gooutput.WriteString(fmt.Sprintf("package binding_%s\n\n", packageName))
			gooutput.WriteString("import \"github.com/maxence-charriere/go-app/v9/pkg/app\"\n\n")
...
`)
			...
			for _, methodName := range methodNames {
				...
				// Generate Go
				gooutput.WriteString(fmt.Sprintf("\nfunc %s(ctx app.Context, %s any) app.Value {", methodName, argsString))
				gooutput.WriteString("\n")
				gooutput.WriteString(fmt.Sprintf("  return app.Window().Get(\"go\").Get(\"%s\").Get(\"%s\").Call(\"%s\", %s)", packageName, structName, methodName, argsString))
				gooutput.WriteString("\n}\n")
			...
			gofilename := filepath.Join(packageDir, structName+".goapp.go")
			fmt.Println("Writing:", gofilename)
			err = os.WriteFile(gofilename, gooutput.Bytes(), 0o755)
			if err != nil {
				return err
			}
